...
import "./interfaces/IUnitasMinting.sol";

/**
 * @title Unitas Minting
 * @notice This contract mints and redeems USDu in a single, atomic, trustless transaction
 */
contract UnitasMinting is IUnitasMinting, SingleAdminAccessControl, ReentrancyGuard {
  ...
  /// @notice USDu minted per block
  mapping(uint256 => uint256) public mintedPerBlock;
  /// @notice USDu redeemed per block
  ...
  /// @notice max minted USDu allowed per block
  uint256 public maxMintPerBlock;
  /// @notice max redeemed USDu allowed per block
  uint256 public maxRedeemPerBlock;

  /* --------------- MODIFIERS --------------- */

  /// @notice ensure that the already minted USDu in the actual block plus the amount to be minted is below the maxMintPerBlock var
  ...
  /// @notice ensure that the already redeemed USDu in the actual block plus the amount to be redeemed is below the maxRedeemPerBlock var
    ...
    belowMaxRedeemPerBlock(order.usdu_amount)
  {
    ...
    redeemedPerBlock[block.number] += order.usdu_amount;
  ...
  function addSupportedAsset(address asset) public onlyRole(DEFAULT_ADMIN_ROLE) {
    if (asset == address(0) || asset == address(usdu) || !_supportedAssets.add(asset)) {
  ...
  function verifyOrder(Order calldata order, Signature calldata signature) public view override returns (bool, bytes32) {
    ...
    if (order.usdu_amount == 0) revert InvalidAmount();
