...
contract Vault is IVault, Pausable, AccessControl, ReentrancyGuard {
  ...
  using SafeERC20 for IShareToken;
  ...
  //ShareTokens list
  mapping(address => IShareToken) public supportedTokenToShareToken;
  mapping(address => address) public shareTokenToSupportedToken;

  uint256 public lastClaimQueueID = 11_000;
  ...
  constructor(
    ...
    for (uint256 i = 0; i < len; i++) {
      ...
      IShareToken shareTokenTemp = IShareToken(_shareTokens[i]);
      supportedTokenToShareToken[token] = shareTokenTemp;
      shareTokenToSupportedToken[address(shareTokenTemp)] = token;
      emit ShareTokenRegistered(address(shareTokenTemp), token);
  ...
  function stake_66380860(
    ...
    supportedTokenToShareToken[_token].mint(msg.sender, mintAmount);
  ...
  function requestClaim_8135334(
    ...
    uint256 shareBalance = supportedTokenToShareToken[_token].balanceOf(msg.sender);

    if (sharesToBurn > shareBalance || assetsInfo.stakedAmount == 0) sharesToBurn = shareBalance;

    supportedTokenToShareToken[_token].burn(msg.sender, sharesToBurn);
  ...
  function cancelClaim(uint256 _queueId, address _token) external whenNotPaused OnlyCancelEnable nonReentrant {
    ...
    supportedTokenToShareToken[_token].mint(msg.sender, amountToMint);
  ...
  function claim_41202704(uint256 _queueID, address _token) external whenNotPaused nonReentrant {
    ClaimItem memory claimItem = claimQueue[_queueID];
    address token = claimItem.token;
    AssetsInfo storage assetsInfo = userAssetsInfo[msg.sender][token];
    uint256[] memory pendingClaimQueueIDs = userAssetsInfo[msg.sender][token].pendingClaimQueueIDs;
  ...
  function _getExchangeRate(address _token) internal view returns (uint256 exchangeRate) {
    uint256 totalSupplyShareToken = supportedTokenToShareToken[_token].totalSupply();
    if (totalSupplyShareToken == 0) {
      exchangeRate = 1e18;
    } else {
      exchangeRate =
  ...
  function convertToShares(uint256 tokenAmount, address _token) public view returns (uint256 shares) {
    ...
    uint256 exchangeRate = totalSupplyShareToken == 0
      ? 1e18
      : ((totalStaked + totalRewards) * 1e18) / totalSupplyShareToken;
  ...
  function transferOrTransferFrom(
    ...
    uint256 tokenBefore = getShareBalance(from, token);
    require(tokenBefore >= amount, "balance");
    if (msg.sender != from) {
      require(supportedTokenToShareToken[token].allowance(from, msg.sender) >= amount, "allowance");
      supportedTokenToShareToken[token].updateAllowance(from, msg.sender, amount);
      supportedTokenToShareToken[token].safeTransferFrom(from, to, amount);
    } else {
      supportedTokenToShareToken[token].safeTransferFrom(msg.sender, to, amount);
    }
  ...
  // Distribute shares (airdrop/allocation)
  function distributeShares(
    ...
    supportedTokenToShareToken[token].safeTransferFrom(distributorAddr, to, amount);

    AssetsInfo storage assetsInfo = userAssetsInfo[to][token];
    if (enableHistoricalRewards) {
    ...
    emit SharesDistributed(token, to, amount, enableHistoricalRewards);
  }
  ...
  function addSupportedToken(
    ...
    IShareToken shareToken = IShareToken(_shareToken);
    supportedTokenToShareToken[_token] = shareToken;
    shareTokenToSupportedToken[address(shareToken)] = _token;
    emit ShareTokenRegistered(address(shareToken), _token);
  }
  ...
  function setDistributorAddr(address newDistributorAddr) external onlyRole(DEFAULT_ADMIN_ROLE) {
    //allow equal address(0), when we want to disable distribution
  ...
  function _getClaimableRewards(address _user, address _token) internal view returns (uint256) {
    ...
    if (currentRewardRateState.updatedTime <= lastRewardUpdate) {
      ...
      uint256 reward = calculateReward(currentStakedAmount, currentRewardRateState.rewardRate, elapsedTime);
      ...
      for (uint256 i = beginIndex; i < rewardRateLength; i++) {
        ...
        uint256 tempReward = calculateReward(currentStakedAmount, rewardRateArray[i - 1].rewardRate, tempElapsedTime);
  ...
  function getClaimableRewardsWithTargetTime(
    ...
    uint256 reward = calculateReward(assetsInfo.stakedAmount, currentRewardRateState.rewardRate, elapsedTime);
  ...
  function getShareBalance(address _user, address _token) public view onlySupportedToken(_token) returns (uint256) {
    return supportedTokenToShareToken[_token].balanceOf(_user);
  }
