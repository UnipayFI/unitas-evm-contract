...
interface IVault {
  ...
  event ShareTokenRegistered(address indexed shareToken, address indexed underlyingToken);
  ...
  function distributeShares(address token, address to, uint amount, bool enableHistoricalRewards) external;
  ...
  function addSupportedToken(address _token, uint256 _minAmount, uint256 _maxAmount, address _shareToken) external;
  ...
  function getShareBalance(address _user, address _token) external view returns (uint256);
