...
contract LendingMarketHelper is Test, USDuListingHelper, IAStEthEvents, Utils {
  ...
  address constant USDU_ADDRESS = 0xa3D012f6437DB89AE02a8d60536cA8b649F577Ae;
  address constant USDU_OWNER = 0x46a1ea206Ef8EC604155abA33AD3a1E3054E132F;
  ...
  function lendingMarketDeployAndSetup()
    ...
    returns (
      ...
      USDuTokenImplementations memory usduProxies
    )
  {
    bob = vm.addr(0x1DE);
    ...
    vm.label(USDU_OWNER, "usdu owner");
    vm.label(POOL_PROXY, "lending pool");

    vm.startPrank(DEPLOYER);
    usdu = IUSDu(USDU_ADDRESS);
    ...
    DataTypes.ReserveData memory usduReserveData = pool.getReserveData(USDU_ADDRESS);

    usduProxies = USDuTokenImplementations(
      USDuAToken(usduReserveData.aTokenAddress),
      USDuStableDebtToken(usduReserveData.stableDebtTokenAddress),
      USDuVariableDebtToken(usduReserveData.variableDebtTokenAddress)
    );
    ...
    // Set the relation between the aUSDu and the USDu variable debt token
    usduProxies.USDuAToken.setVariableDebtToken(usduReserveData.variableDebtTokenAddress);
    usduProxies.USDuVariableDebtToken.setAToken(usduReserveData.aTokenAddress);

    vm.startPrank(USDU_OWNER);
    grantUSDuMinterRole(usdu, PROTOCOL_DATA_PROVIDER);

    return (usdu, pool, aclManager, contracts, stETH, stEthProxies, usduProxies);
  }
}
